      ...
      return NextResponse.json(
        {
          error: error.message,
          details:
            (error as unknown as Record<string, unknown>).details || null,
        },
        { status: 500 },
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    log("info", "DELETE succeeded", {
      table,
      uuid,
      removed: Array.isArray(data) ? (data as unknown[]).length : 1,
    });
