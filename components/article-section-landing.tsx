...
export function ArticleSectionLanding({
  ...
  const filteredArticles = useMemo(() => {
    ...
    return shuffled.filter((article) =>
      activeCategories.every((category) => article.categories.includes(category))
    )
  ...
  const HeroSection = ({ className }: { className?: string }) => (
    ...
    >
      <div className="space-y-3">
        <p className="text-[0.6rem] uppercase tracking-[0.65em] text-white/45">{sectionLabel}</p>
        <h1
          className={cn(
            playfair.className,
      ...
      {filterActive ? (
        <button
          ...
          className="text-[0.6rem] uppercase tracking-[0.35em] text-white/60 underline"
        >
          Reset filters
        </button>
      ) : null}
      ...
      >
        <Link href={href} className="group block h-full">
          <div className="relative flex h-full min-h-[300px] flex-col justify-between overflow-hidden rounded-[1.35rem] border border-black/10 bg-[#f5efe3] p-7 shadow-[0_14px_36px_rgba(0,0,0,0.28)] transition-transform duration-500 ease-out group-hover:-translate-y-2">
            ...
            <div className="pointer-events-none absolute inset-0 bg-[repeating-linear-gradient(118deg,transparent_0,transparent_12px,rgba(0,0,0,0.025)_12px,rgba(0,0,0,0.025)_13px)]" aria-hidden />
            <div className="pointer-events-none absolute inset-4 rounded-[0.95rem] border border-black/8" aria-hidden />
            <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-6 bg-[radial-gradient(circle_at_top,rgba(0,0,0,0.1),transparent_70%)] opacity-70" aria-hidden />

            <div className="relative z-10 space-y-6 text-left text-neutral-900">
              <div className="space-y-3">
                <p className="text-[0.68rem] uppercase tracking-[0.45em] text-neutral-500">{cardLabel}</p>
                <h3 className={cn(playfair.className, 'text-[1.35rem] font-semibold leading-snug text-neutral-900')}>
                  {article.title}
                </h3>
                {article.categories.length > 0 ? (
                  <div className="flex flex-wrap gap-2 text-[0.65rem] uppercase tracking-[0.3em] text-neutral-500">
                    {article.categories.slice(0, 3).map((category) => (
                      <span key={`${article.uuid}-${category}`} className="rounded bg-neutral-200/80 px-2 py-0.5 text-neutral-600">
                        {category}
                      </span>
                    ))}
                  </div>
  ...
  const SkeletonCard = ({ index, className }: { index: number; className?: string }) => {
    const posture = cardPostures[index % cardPostures.length]
    const tapeAngle = tapeAngles[index % tapeAngles.length]
      ...
      >
        <div className="relative flex h-full min-h-[300px] flex-col justify-between overflow-hidden rounded-[1.35rem] border border-black/10 bg-[#f5efe3] p-7 opacity-70">
          <div
            ...
            aria-hidden
          />
          <div className="pointer-events-none absolute inset-0 bg-[repeating-linear-gradient(118deg,transparent_0,transparent_12px,rgba(0,0,0,0.015)_12px,rgba(0,0,0,0.015)_13px)]" aria-hidden />
          <div className="pointer-events-none absolute inset-4 rounded-[0.95rem] border border-black/8" aria-hidden />
          <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-6 bg-[radial-gradient(circle_at_top,rgba(0,0,0,0.08),transparent_70%)] opacity-70" aria-hidden />
  ...
  const secondRowCards = filterActive ? filteredArticles.slice(0, 3) : shuffled.slice(2, 5)
  ...
  return (
    <div className={cn(manrope.className, 'relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.12),_rgba(0,0,0,0.92)_55%)]')}>
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-24 pt-28 sm:px-10">
        <div className="hidden w-full gap-14 lg:grid">
          <div className="grid grid-cols-3 gap-10">
            {isLoading ? (
              ...
              <>
                {firstRowLeft ? <ArticleCard article={firstRowLeft} index={0} /> : <div />}
                <HeroSection className="h-full w-full justify-center" />
                {firstRowRight ? <ArticleCard article={firstRowRight} index={1} /> : <div />}
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-10">
            {isLoading ? (
              skeletonIndices.slice(2, 5).map((idx) => <SkeletonCard key={`skeleton-${idx}`} index={idx} />)
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-3 items-start gap-10">
              <div />
              <div className="flex justify-center">
                <ArticleCard article={bottomRowCard} index={5} className="w-full max-w-sm" />
              </div>
              <div />
            </div>
          ) : null}
        ...
        <div className="flex flex-col gap-12 lg:hidden">
          <HeroSection />
          {isLoading ? (
            skeletonIndices.map((idx) => <SkeletonCard key={`mobile-skeleton-${idx}`} index={idx} />)
          ) : mobileHasItems ? (
            mobileStack.map((article, index) => (
              <ArticleCard key={article.uuid ?? `${article.title}-${index}`} article={article} index={index} />
            ))
          ) : (
